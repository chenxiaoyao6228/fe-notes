---
title: 使用vuepress搭建静态网站
categories:
  - tech
tags:
  - node
date: 2019-11-25
permalink: 2019-11-25-build-your-website-with-vuepress
---

## 起因

旧的 hexo 主题不怎么好看, vuepress 的默认主题清爽, 也方便自己 DIY, 于是开始了改造活动. 过程中有踩了一点坑, 也留意到一些之前没有留意到的东西, 记录一下.

## 两种 github page

在 vuepress 中有下面两行文档

```js
# if you are deploying to https://<USERNAME>.github.io
# git push -f git@github.com:<USERNAME>/<USERNAME>.github.io.git master

# if you are deploying to https://<USERNAME>.github.io/<REPO>
# git push -f git@github.com:<USERNAME>/<REPO>.git master:gh-pages
```

每个账户可以建立一个 username.github.io 的的仓库用来托管个人网站, 代码只能够放在 master 分支上, 如下面的仓库可以通过 chenxiaoyao6228.github.io 进行访问
![2019-11-25-04-20-45](https://cdn.jsdelivr.net/gh/chenxiaoyao6228/cloudimg@main/2019/10/2019-11-25-04-20-45.png)

另外一种则是为每个项目单独设立的仓库, 比如你写了一个组件库, 主代码托管在 master 分支, 展示的代码放在 gh-page 分支上, 但是访问只能是`chenxiaoyao6228.github.io/one-ui`的方式

![2019-11-25-04-29-53](https://cdn.jsdelivr.net/gh/chenxiaoyao6228/cloudimg@main/2019/10/2019-11-25-04-29-53.png)

在开发自己的 vuepress 博客的时候,自己的仓库名是`blog`,选择第一种,生成的静态文件会 push 到`chenxiaoyao6228.github.io`仓库的 master 分支上,反之, 则挂在 blog 仓库的 master 和 gh-page 目录下

同时也需要注意第二种情况下需要加 base 字段到

```js
Set correct base in docs/.vuepress/config.js.

If you are deploying to https://<USERNAME>.github.io/, you can omit base as it defaults to "/".

If you are deploying to https://<USERNAME>.github.io/<REPO>/, (that is your repository is at https://github.com/<USERNAME>/<REPO>), set base to "/<REPO>/".
```

> Update: 本地上来说，是 github 为这些路径做反向代理不同导致，如果你有将项目部署在二级路由下的经验的话，就会理解 vuepress 的设置实际上是 webpack 的 publicPath

## 自定义域名

### 生成 cname 文件

```bash
#  生成cname文件, 用于自定义域名, 不要http, www开头, 切记!
 echo 'chenxiaoyao.cn' > CNAME
```

### 多个 gh-page 使用相同的域名

一种办法是使用子域名来解决, 如 blog.chenxiaoyao.cn

## travis 持续集成

每次写完博客后，都需要先 push 代码到仓库, 然后自己在本地 run build 将 Markdown 转换成页面，再 push 到仓库的 gh-pages 分支, 有没有什么办法可以实现 push 代码之后自动部署呢?

持续集成工具可以很好地解决这个问题, 常见的工具如 circle-ci, travis-ci. vuepress 使用了 travis-ci 作为例子.

### bash 命令

```bash
set -e

# 生成静态文件
npm run build
cd .vuepress/dist

#  生成cname文件, 用于自定义域名, 不要http, www开头, 切记!
 echo 'chenxiaoyao.cn' > CNAME

git init
git add -A
git commit -m 'deploy'

#  ${GITHUB_TOKEN}为travis的deploy配置, 在配置的时候使用 $GITHUB_TOKEN
git push -f https://${GITHUB_TOKEN}@github.com/chenxiaoyao6228/blog.git master:gh-pages

cd -

rm -rf .vuepress/dist
```

### travis 配置

travis 集成的时候的时候显示 126 错误, 有两种原因, 一是不是可执行文件, 如文件第一行没有加`#!/usr/bin/env sh`, 第二种使用下面的方法赋予权限

```yml
script:
  - chmod +x ./deploy.sh
```

$为 bash 使用时候的变量标志,通过命令行设置的时候不要加美元符号

```yml
deploy:
  github-token: "$GITHUB_TOKEN"
```

![2019-11-25-05-05-28](https://cdn.jsdelivr.net/gh/chenxiaoyao6228/cloudimg@main/2019//10/2019-11-25-05-05-28.png)

使用上面的命令会生成下面的字段

```yml
env:
  global:
    - secure: 
```

更多的看[官网](https://docs.travis-ci.com/user/environment-variables#defining-variables-in-repository-settings)

### vuepress 构建错误

```js
TypeError: Cannot read property 'match' of undefined
    at getHash (node_modules/@vuepress/theme-default/util/index.js:13:0)
    at isActive (node_modules/@vuepress/theme-default/util/index.js:47:0)
    at render (node_modules/@vuepress/theme-default/components/SidebarLink.vue:25:32)
```

<https://github.com/vuejs/vuepress/issues/570>
